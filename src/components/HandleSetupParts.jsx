...
const useSchema = yup.object().shape({
  ...
  productLink: yup.string().required('This is required'),
  imgLink: yup.string().required('This is required'),
});

export default function HandleSetupParts({ selectedRow, updateList }) {
    ...
    if (clickedType === 'edit') {
      const value = async () => {
        try {
          ...
          const resSetupPart = await fetch('/api/setupParts');
          const jsonSetupPart = await resSetupPart.json();
          setIsEdit(true);
          await jsonSetupPart.data.map((setupItem) => {
            if (setupItem._id === selectedTable) {
              initialValues = {
                _id: setupItem._id,
                name: setupItem.name,
                productLink: setupItem.productLink,
                imgLink: setupItem.imgLink,
              };
              setOpen(true);
            } else {
              return;
  ...
  const handleAddSetupParts = async (values) => {
    let fieldValues = {};
    if (isEdit) {
      fieldValues = {
        _id: values._id,
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  >
                    ...
                    <TextField
                      ...
                      name="productLink"
                      error={!!touched.productLink && !!errors.productLink}
                      helperText={touched.productLink && errors.productLink}
                      sx={{ gridColumn: 'span 4' }}
                    />
                    <TextField
                      fullWidth
